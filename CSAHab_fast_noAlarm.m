function [strf, fdat]=CSAHab_fast_noAlarm(Tfinal,Np,kmax, sd, Rcrit, a, seq_data, a0)%Number frames, number ants, max oc kick distribution, decay rate of speed param toward Veq, interaction radius, bug Lambda param
...
% Np=62;%number of ants in total population
...
% plot(lst(:,1),lst(:,2),'o')
% Rcrit=50;%(in pxls) interaction distance 40*mppx=5.8mm

%Velocity modulation params---------------
% seq_data=1.8739; %mean speed over all ants pxl/frame. used as the equilib of the speed params
Veq=seq_data;%equilibrium velocity, (pxl/frame)
% Vhwidth=0.5*Veq;
% V1=seq_data; %velocity of Alarmed seeds = 8.7500 (pxl/frame) 
...
% sig_sr=0.04;%random walk step std dev
% mu_sr=0.03;%mean abs value change in speed due to noise
% sd=0.001;%slope of natural decay function
...
%set initial speed parameter distribution:
% lst=[(1:Np)' lst, randintr(Veq-Vhwidth, Veq+Vhwidth, Np,1), zeros(Np,1)];%=[id x y velocity_param, alarm state(0/1/2), vx, vy]
% lst=[(1:Np)' lst, 0+random('exp',Veq, [Np,1]), zeros(Np,1)];%mean of exponential distribution with param lambda is 1/lambda
...
% Tinject=2;%time of injection of alarmed ants
...
%MOVEMENT STEP
    for p=1:Np%move all workers in this frame. interaction comes later
     ...
     TH2=TH+(round(rand(1))*2-1)*random('exp', theta_bias);%random deviation from  with slight bias to the right
...
%          TH2=TH+(mod(p,2)*2-1)*random('exp', theta_bias);%random deviation from straight. clockwise if even ID or counterclock if odd ID
         TH2=TH+(round(rand(1))*2-1)*random('exp', theta_bias);%random deviation from straight. clockwise if even ID or counterclock if odd ID
...
if isempty(cind)==1%store all contacts from this frame
    ...
    activ_log_j=sum(IDpairs(:,3:4)==repmat([1 0], size(IDpairs,1), 1),2)==2;
